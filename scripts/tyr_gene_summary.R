...
plot_a <- ggplot(tyr_genes, aes(x = length_kb)) +
  geom_histogram(binwidth = 0.2, fill = "steelblue", alpha = 0.8) +
  geom_vline(xintercept = largest_tyr$length_kb, color = "coral2", size = 1, linetype = "dashed") +
  labs(title = "A. Length of Predicted tyr Genes",
       x = "Gene Length (kilobases)", y = "Count") +
  theme_minimal()

# Plot B: tyr gene positions on scaffolds
plot_b <- ggplot(tyr_genes, aes(x = start / 1e6, y = reorder(scaffold, start), color = strand)) +
  geom_point(size = 2) +
  labs(title = "B. Genomic Positions of tyr Genes",
       x = "Position on Scaffold (Megabases)", y = "Scaffold",
       color = "Strand") +
...
plot_c <- ggplot(neighborhoods, 
  ...
  labs(title = "C. Genomic Neighborhood Sizes",
       x = "Neighborhood ID", 
       y = "Size (kilobases)",
       subtitle = paste("Largest tyr gene is in", containing_neighborhood)) +
  coord_flip() +
  theme_minimal()
...
# Save the figure
figure_path <- "../figures/tyr_gene_summary.png"
